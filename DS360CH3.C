...
main()
{
 ...
 //PP=P+P_II;
	 ...
	 //
	 //
	 OUT=fopen("ds360chk.out","r");
	 fgets(MMMM,80,OUT);
	 fclose(OUT);

	 
	 if (MMMM[19]>='0' && MMMM[19]<='9') SER_NUM=MMMM[19]-'0';
	 else SER_NUM=MMMM[19]-'a'+10;
	 if (MMMM[24]>='0' && MMMM[24]<='9') SER_NUM+=(MMMM[24]-'0')<<4;
	 else SER_NUM+=(MMMM[19]-'a'+10)<<4;

	 SER_NUM++;

	 printf("\n ‚‚…„ˆ’… ˆ—ˆ“ “„€‹…Ž‰ …ƒˆ‘’€–ˆˆ(BY DS360CH3):");
	 gets(MMMM);
	 printf("\n ENTER CHECK STRING:\n");
	 for(i_count=0;i_count<5;i_count++)
		 {scanf("%02x",&i_count_2);
			sprintf(&bios_32[i_count*2],"%02x",i_count_2);
		 }
	 for(all_count=0,i_count=5,BIOS_AREA=0xf000fff5;i_count<16;i_count++)
		 {
			scanf("%02x",&i_count_2);
			all_count+=((unsigned long)(i_count_2))<<offset_count[i_count];
			sprintf(&bios_32[i_count*2],"%02x",i_count_2);
		 }
	 printf("\n%08lx",all_count);
	 sprintf(&bios_32[32],"%08lx",all_count);
	 for(all_count=0,i_count=0,i_count_2=0;i_count<40;i_count++)
		 {
			all_count+=((unsigned long)(bios_32[i_count]))<<offset_count[i_count_2];
			if ((++i_count_2)>15) i_count_2=0;
		 }
	 printf("\n%08lx",all_count);
	 sprintf(&bios_32[40],"%08lx",all_count);
	 printf("\n\n%s",bios_32);
	 OUT=fopen("ds360chk.out","w");
	 fprintf(OUT,"SERIAL NUMBER=SDRIT%xOEGy%x",SER_NUM&0x000f,SER_NUM>>4);
	 time(&t);
	 fprintf(OUT,"\nDATE OF REGISTRATION=%s",ctime(&t));
	 fprintf(OUT,"REGISTRATION REASON:%s",MMMM);
	 fprintf(OUT,"\n*************************************************************");
	 fprintf(OUT,"\nLN=%s\n‚‘… ‘’ŽŠˆ ‚›˜… „€Ž‰ … ŒŽƒ“’ ›’œ ˆ‡Œ……› …‡",bios_32);
	 fprintf(OUT,"\n€‡…˜…ˆŸ €‡€Ž’—ˆŠ€ (0622)-(35-41-28)/(90-44-07)");
	 fprintf(OUT,"\n‹žŽ… ˆ• ˆ‡Œ……ˆ… ŒŽ†…’ ‚›‡‚€’œ ……„‘Š€‡“…Œ›‰ …‡“‹œ’€’ ‚ €Ž’… Œ“‹Ÿ’Ž€");
	 fprintf(OUT,"\n*************************************************************");
	 fclose(OUT);

	 HISTORY=fopen("HISTORY","a");
	 fprintf(HISTORY,"SERIAL NUMBER=SDRIT%xOEGy%x",SER_NUM&0x000f,SER_NUM>>4);
	 fprintf(HISTORY,"\nDATE OF REGISTRATION (remoute by ds360ch3)=%s",ctime(&t));
	 fprintf(HISTORY,"REGISTRATION REASON:%s",MMMM);
	 fprintf(HISTORY,"\nLN=%s\n************************************************\n",bios_32);
	 fclose(HISTORY);
